...
module sysid_host_tb;
  ...
  top dut(.clk_25mhz(clk),.ftdi_txd(1'b1));
  integer guard,k,w;
  initial begin
    force dut.clk=clk;
  ...
  task access(input [31:0] addr,input wr,input bad);
    begin
      ...
      if(!bad && addr==32'h80000008 && dut.mem_read_word!==32'd6)
        $fatal(1,"SYSTEM_ID incorrecto %h",dut.mem_read_word);
      if(!bad && addr==32'h8000000c && dut.mem_read_word!==32'h00000009)
        $fatal(1,"DEVICES incorrecto %h",dut.mem_read_word);
      if(!bad && addr==32'h80000014 && dut.mem_read_word!==32'h00008000)
        $fatal(1,"MEM_SIZE incorrecto %h",dut.mem_read_word);
      if(!bad && addr==32'h80000018 && dut.mem_read_word!==32'h00000306)
