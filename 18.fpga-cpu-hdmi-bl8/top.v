...
module top (
  ...
  // El bufer de instrucciones se intercala aqui: mismo contrato en sus dos
  // lados, asi que el adaptador no se entera de que existe. Lo que ve el
  // adaptador son estas senales `buf_*`.
  wire buf_imem_valid, buf_imem_ready;
  wire [31:0] buf_imem_address, buf_imem_read_data;
  ...
  sdram_system_adapter adapter_i(
      ...
      .cpu_imem_valid(buf_imem_valid), .cpu_imem_address(buf_imem_address),
      .cpu_imem_read_data(buf_imem_read_data), .cpu_imem_ready(buf_imem_ready),
  ...
  // Cuatro lineas de 16 bytes entre la CPU y el adaptador. Se instancia aqui,
  // despues del adaptador, solo porque `init_done` se declara con el.
  //
  // El vaciado cuelga de `cpu_halted`, no de `reset`: el monitor reescribe la
  ...
  //
  // Los contadores de aciertos y fallos no se conectan: existen para los
  // bancos de prueba y la sintesis los quita.
