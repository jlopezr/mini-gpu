...
module top (
  ...
  // cabian dentro; ahora estan a megabytes unos de otros y hacen falta varias.
  ...
  // aqui.
  //
  // Son TRES y no cuatro: esta carpeta no tiene puerto serie (HAS_SERIAL(0)
  // en el decodificador), asi que el bloque SERIAL no existe y abrirle una
  // ventana solo dejaria pasar accesos que el decodificador va a rechazar.
  //
  ...
  wire mmio_error;
  wire mmio_video_error, mmio_perf_select;
  ...
  // Modo de salida, de video_registers al mux de fuentes de linea. Se
  // declara aqui porque el bloque de registros se instancia mas abajo que
  // el mux.
  wire [1:0] video_mode;
  wire [7:0] fill_line;
  wire [23:0] fb_base;
  ...
  // placa solo se recuperaba reprogramando la FPGA. Lo comprueba
  ...
  //
  // ISA_PROFILE: MUL y DIV. Ni subpalabra --esta carpeta no tiene STOREB/LOADB--
  // ni el bit 3 de SIMT, que aunque `cpu.v` decodifique SSY y BAR aqui son
  // NO-OP, puestos para poder compartir binarios con la GPU.
  //
  // Esta carpeta no tiene puerto serie: su `select` se queda sin conectar y su
  // dato leido es cero, que es lo mismo que devuelve cualquier dispositivo que
  // no existe.
  //
  // DEVICES (§5.4): SYSTEM(0) + SDRAM(2) + VIDEO(5) + CPU(9). SIN el bit 4
  // (SERIAL), que es lo unico que lo separa del de la 19. No hay FABRIC
  // declarado --lo hay fisicamente, pero su bloque no tiene registros-- ni
  // EBR, ni TIMER, ni INTC, ni DMA, ni GPU.
  //
  //   bit 9 CPU | bit 5 VIDEO | bit 2 SDRAM | bit 0 SYSTEM
  //   0000_0010_0010_0101 = 0x0225
  //
  ...
  //
  // VIDEO_REGISTERS pasa de 0x7f a 0x3ff, y no es cosmetico: `video_registers.v`
  // implementa ahora DIEZ registros y dejarlo en siete haria que HALT_AT,
  // HALT_TARGET y VIDEO_TX contestaran error de acceso aunque el RTL los tiene.
  // Es el fallo que la 19 encontro en la 21 ya sintetizada, y sobrevive callado
  // porque 0x7f sigue siendo un bitmap perfectamente valido.
  wire mmio_video_select;
  wire [31:0] mmio_video_read_data;
  wire [31:0] mmio_perf_read_data;
  mmio_decoder #(.FOLDER(8'd18), .HAS_SERIAL(0), .VIDEO_REGISTERS(64'h3ff),
      .ISA_PROFILE(32'h0000_0003),
      .DEVICES(32'h0000_0225),
      .MEM_BASE(32'h0000_0000), .MEM_SIZE(32'h0200_0000),
      .MONITOR_VERSION(32'h0000_0312))   // 3.18, el mismo que monitor_i
    mmio_decoder_i(
