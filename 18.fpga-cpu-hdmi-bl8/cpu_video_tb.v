...
module cpu_video_tb;
  ...
  // El mismo reparto de ventana que top.v, `HAS_SERIAL(0)` incluido: lo que se
  // prueba aqui es el mapa que se sintetiza, y en esta carpeta ese mapa NO
  // tiene puerto serie. Ponerlo a uno probaria un decodificador que no es el
  // del bitstream, que es el fallo que la 19 encontro en `cpu_mmio_error_tb`.
  mmio_decoder #(.HAS_SERIAL(0)) mmio_decoder_i (
  ...
  // Sin `serial_port`: esta carpeta no tiene ese fichero. `mmio_serial_select`
  // se queda observado y siempre a cero, que es lo que `HAS_SERIAL(0)` obliga.
