...
module cpu_tb;
  ...
  // take six cycles, registered ALU operations seven, BRA seven,
  // conditional branches eight, and shifts
  // 7 + la cantidad de desplazamiento. LOAD/STORE are excluded because their
  // data-memory latency is intentionally defined by the valid/ready handshake.
  //
  // MULHI, REM y REMU NO aparecen en esta tabla: su latencia depende de si
  // aciertan la etiqueta del medio resultado, y fijarla aqui obligaria a
  // duplicar en el testbench la logica que se quiere probar. Lo hace
  // alu_fast_path_tb.v, que las mide contra la misma CPU con el atajo
  // desarmado en lugar de contra un numero escrito a mano.
  always @(posedge clk) begin
    if (reset) begin
      ...
      if (dut.state == 4'd4) begin
        ...
        // La cantidad puede venir de Rb o del propio encoding: `shift_amount`
        // es el mux que las une, y usarlo aqui mantiene la prediccion valida
        // para SHLI/SHRI/SARI sin repetir la condicion.
        if (dut.opcode >= 6'h07 && dut.opcode <= 6'h09)
          expected_instruction_cycles = 7 + dut.shift_amount;
        ...
        // 0x15/0x16 y el bloque 0x18..0x1D son accesos a memoria: su latencia
        // la fija el handshake valid/ready, no el numero de estados. MULHI
        // (0x0B), REM (0x0E) y REMU (0x0F) quedan fuera por el camino rapido:
        // ver el comentario de arriba.
        if (dut.opcode != 6'h15 && dut.opcode != 6'h16 &&
            dut.opcode != 6'h0b && dut.opcode != 6'h0e && dut.opcode != 6'h0f &&
            !(dut.opcode >= 6'h18 && dut.opcode <= 6'h1d) &&
  ...
  initial begin
    ...
    // que desde la 21 esta cableado a cero: la escritura se descarta y lo que
    // demuestra el caso es que STEP retiro una instruccion, no cual. El detalle
    // de R0 tiene su propio testbench, zero_register_tb.v.
    ...
    // divergente sin SSY delante para el SM con ERROR_SIMT, asi que no se pueden
    // quitar del fuente compartido.
    ...
    if (retired_count !== 4) $fatal(1, "SSY/BAR retired count mismatch");

    // Y el no-op es ESTRECHO: EXIT (0x33) es de GPU y sigue siendo opcode
    // invalido, igual que 0x34-0x3d. Solo se aceptan los dos que tienen
    // traduccion trivial a una maquina de un hilo.
