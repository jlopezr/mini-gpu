...
> **Actualización de la 21.** Este documento llegó de la 19, donde `R0` era un
> registro general y por eso `JR` gastaba un opcode propio. Aquí **`R0` está
> cableado a cero**, con lo que ese día que el párrafo siguiente da por lejano ya
> ha llegado: `JR Ra` es exactamente `JALR R0, Ra, 0`.
>
> `JR` **no se ha quitado**. Sigue implementado y sigue siendo válido; queda
> marcado como obsoleto y su hueco (`0x2E`) como **reclamable**, no libre.
> Quitarlo hoy rompería todo lo que usa `RET` sin ganar nada: el opcode no hace
> falta todavía. Cuando haga falta, basta con cambiar el alias del ensamblador y
> reensamblar. Ver [`registro-cero.md`](registro-cero.md).
>
> Lo demás del párrafo sigue en pie: seguimos en el mapa de la v0.2, con
> `JAL`/`JALR` en `0x2C`/`0x2D` y no en `0x23`/`0x24`.
