...
module video_mode_switch_tb;
  ...
  // ------------------------------------------------------------ bus de juguete
  wire req_valid, req_write, req_urgent;
  wire [31:0] req_addr;
  wire [127:0] req_wdata;
  wire [15:0] req_wmask;
  // `rsp_ready` es una SALIDA de la fuente: hay que respetarla. Ignorarla hace
  // que una respuesta se pierda cuando la fuente no la acepta, y entonces la
  // linea no termina nunca -- que se parece mucho al fallo que este banco busca
  // y llevaria a acusar al mux de algo que es del modelo.
  reg rsp_valid = 0;
  wire rsp_ready;
  ...
  video_line_source_burst source_sdram_i(
      ...
      .req_valid(req_valid), .req_ready(1'b1), .req_write(req_write),
      .req_addr(req_addr), .req_wdata(req_wdata), .req_wmask(req_wmask),
      .urgent(req_urgent),
      .rsp_valid(rsp_valid), .rsp_ready(rsp_ready),
      .rsp_rdata(128'h0123_4567_89ab_cdef_0123_4567_89ab_cdef),
      .rsp_error(1'b0));

  // Una respuesta por peticion aceptada, sostenida hasta que la fuente la toma.
  always @(posedge clk) begin
    if (src_reset) begin
      rsp_valid <= 1'b0;
    end else begin
      if (rsp_valid && rsp_ready) rsp_valid <= 1'b0;
      if (req_valid && !req_write) rsp_valid <= 1'b1;
    end
  end
