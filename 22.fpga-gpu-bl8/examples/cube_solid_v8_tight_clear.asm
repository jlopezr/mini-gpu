; Cubo solido v8: borrado ajustado a la envolvente del cubo.
;
; Conserva la geometria SIMD y el setup compacto de v6, pero limpia solamente
; xword=24..135, y=16..223: 23.296 palabras frente a 28.672 (-18,75%).
...
geometry_ready:
        ...
        ; Cada warp posee una fila; sus lanes cubren bloques de ocho palabras.
        ANDI  R2, R1, 7
        MOVI  R8, 3
        SHR   R3, R1, R8
        ADDI  R4, R3, 16          ; y inicial = 16 + warp
frame_clear_row:
        MOVI  R5, 160
        MUL   R5, R4, R5
        ADDI  R5, R5, 24          ; primera palabra x, alineada a ocho
        ADD   R5, R5, R2
        ADD   R5, R5, R5
        ADD   R5, R5, R5
        ADD   R5, R19, R5
        MOVI  R6, 14              ; 14 bloques * 8 lanes = 112 palabras
frame_clear_block:
        STORE R0, R5, 0
        ADDI  R5, R5, 32
        ADDI  R6, R6, -1
        BNE   R6, R0, frame_clear_block
        ADDI  R4, R4, 8
        MOVI  R8, 224
        BLT   R4, R8, frame_clear_row
        BAR
