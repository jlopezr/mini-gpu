...
module gpu_system_bl8_tb;
  ...
  reg [31:0] host_write_word=0; reg host_write_word_enable=0;
    ...
    // Instrumentacion: ciclos de emision por puerto del fabric. p0 es la LSU
    // vectorial, p1 el fetch (bufer de instrucciones), p3 el host. Sirve para
    // saber quien manda el trafico antes de decidir donde optimizar -- y la
    // respuesta resulto ser el fetch, por tres ordenes de magnitud.
    integer tx_lsu=0, tx_imem=0, tx_host=0;
    always @(posedge clk) if(!reset) if(dut.fabric.state==2'd1) begin
        if(dut.fabric.active_master==2'd0) tx_lsu=tx_lsu+1;
        if(dut.fabric.active_master==2'd1) tx_imem=tx_imem+1;
        if(dut.fabric.active_master==2'd3) tx_host=tx_host+1;
    end
    task access;
        input wr;
        input [31:0] addr;
    ...
    initial begin
        ...
        $display("TX lsu=%0d imem=%0d host=%0d  (hits=%0d misses=%0d)",tx_lsu,tx_imem,tx_host,dut.imem_hits,dut.imem_misses);
        $display("PASS all differential GPU cases"); $finish;
    end
    initial begin #200000000; $fatal(1,"watchdog"); end
endmodule

// El modelo de SDRAM (sdram_model.v, el de 21, con rafagas) entra como fuente.
