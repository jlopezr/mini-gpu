...
module monitor (
  ...
  localparam [4:0] S_IDLE=0, S_ADDRESS=1, S_WRITE_DATA=2,
    ...
    S_SEND=11, S_WAIT_TX=12, S_VALIDATE_LENGTH=13;
  ...
  wire [32:0] block_end = {1'b0, mem_address} + {17'b0, block_length};
    ...
    if (reset) begin
      ...
      case (state)
        ...
        S_LENGTH_LOW: if (rx_strobe) begin
          ...
          state<=S_VALIDATE_LENGTH;
        end
        S_VALIDATE_LENGTH: begin
          if (block_length==0 || block_length>256 || block_end>33'h02000000) begin
            response_0<=RSP_ERROR; response_length<=1; response_index<=0;
            send_return_state<=S_IDLE; state<=S_SEND;
