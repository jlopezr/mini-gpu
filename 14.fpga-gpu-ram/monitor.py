...
# `top.v`: las dos listas tienen que decir lo mismo, y hay un test que lo
# comprueba. Si añades una ventana aquí y no allí, el CLI la ofrece y la placa
# la rechaza.
#
# MMIO v2: una ventana por bloque presente, y cada final es el del contenido
# real del bloque, no los 64 KiB que ocupa en el mapa.
...
class MonitorClient(WarpMixin, protocolo.MonitorClient):
    ...
    # `WarpMixin` sigue siendo el de v1 (0x80000100) porque tres carpetas de
    # esta familia todavía no están migradas. Sin esta línea, `select_context`
    # escribiría en una dirección que este bitstream rechaza, y el síntoma
    # sería «registros del warp equivocado», no un error.
