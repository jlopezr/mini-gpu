...
module gpu_lsu (
    ...
    localparam IDLE=0, SEND_LOW=1, WAIT_LOW=2, SEND_HIGH=3,
        WAIT_HIGH=4, AUX_RESPONSE=5, VECTOR_RESPONSE=6;
    reg [2:0] state, cursor, selected, selected_lane;
    reg [7:0] busy, pending[0:7], errors[0:7];
    ...
    reg [31:0] lane_address;
    integer k,l,i;
    always @* begin
        found=0; pick=cursor; candidate=0;
        for(k=0;k<8;k=k+1) begin
            candidate=cursor+k[2:0];
            if(!found && busy[candidate] && (pending[candidate]!=0 || !rsp_valid)) begin found=1; pick=candidate; end
        end
        lane_found=0; lane_pick=0;
        for(l=0;l<8;l=l+1)
            if(!lane_found && pending[pick][l]) begin lane_found=1; lane_pick=l[2:0]; end
        lane_address=addresses[pick][lane_pick*32 +: 32];
    end
    ...
    always @(posedge clk) begin
        if(reset) begin
            state<=IDLE; cursor<=0; selected<=0; selected_lane<=0; busy<=0;
            ...
            if(req_valid && req_ready) begin
                busy[req_tag]<=1; pending[req_tag]<=req_mask; errors[req_tag]<=0;
            ...
            case(state)
                IDLE: if(init_done) begin
                    if(take_aux) begin
                        ...
                        if(!lane_found) begin
                            // selected is registered first; collect its lane buffers next cycle.
                            state<=VECTOR_RESPONSE;
                        end else if(|lane_address[31:25] || |lane_address[1:0]) begin
                            pending[pick][lane_pick]<=0; errors[pick][lane_pick]<=1; cursor<=pick+1'b1;
                        end else begin
                            owner_aux<=0; word_address<=lane_address;
                            word_data<=values[pick][lane_pick*32 +: 32];
                            word_strobe<=stores[pick] ? 4'hf : 4'h0; state<=SEND_LOW;
                        end
                    end
                end
                ...
                WAIT_HIGH: if(mem_done) begin
                    if(owner_aux) begin
                        ...
                        pending[selected][selected_lane]<=0; cursor<=selected+1'b1; state<=IDLE;
                    end
                end
