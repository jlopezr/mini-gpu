  ...
  always @(posedge clk) begin
    if (reset) begin
      ...
      case (state)
        ...
        ST_INIT_PRE_WAIT:
          if ({16'b0,timing_count}+1>=TRP_CYCLES) begin timing_count<=0; state<=ST_INIT_REF; end
        ...
        ST_INIT_REF_WAIT1:
          if ({16'b0,timing_count}>= TRFC_CYCLES-1) begin
            timing_count<=0;
        ...
        ST_INIT_MRS_WAIT1:
          if ({16'b0,timing_count}>= TMRD_CYCLES-1) begin
        ...
        ST_TRCD:
          if ({16'b0,timing_count}+1>=TRCD_CYCLES) begin
        ...
        ST_READ_WAIT1:
          if ({16'b0,timing_count}>= CAS_LATENCY_CYCLES-1) begin timing_count<=0; state<=ST_READ_CAPTURE; end
          else timing_count<=timing_count+1'b1;
        ...
        ST_WRITE_WAIT1:
          if ({16'b0,timing_count}>= TWR_CYCLES-1) begin timing_count<=0; done<=1; state<=ST_IDLE; end
          else timing_count<=timing_count+1'b1;
        ...
        ST_REFRESH_WAIT1:
          if ({16'b0,timing_count}>= TRFC_CYCLES-1) begin timing_count<=0; state<=ST_IDLE; end
          else timing_count<=timing_count+1'b1;
