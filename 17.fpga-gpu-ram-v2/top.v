...
module top(input clk_25mhz, output [7:0] led, output wifi_gpio0,
    ...
    //
    // Las ventanas son la GEMELA de MONITOR_REGIONS en monitor.py, y en MMIO
    // v2 hay una por bloque presente. Esta no tiene video ni serie, asi que la
    // ranura que sobra va al centinela.
    //
    // El final es EXCLUSIVO y se ajusta a lo que el bloque implementa de
    // verdad, no a los 64 KiB que el bloque ocupa en el mapa: abrir el bloque
    // entero dejaria que el monitor pidiera direcciones que el decodificador
    // rechaza, y el NACK llegaria del bus en vez de del monitor.
    monitor #(.VERSION_MAJOR(8'd3),.VERSION_MINOR(8'd17),
